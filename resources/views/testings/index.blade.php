                        ...
                        @foreach($templates as $template)
                            @if($template->type == 1)
                                ...
                                <div id="template_{{ $template->order }}" class="main_view_content">
                                    ...
                                    @if(0)
                                        <img src="{{ $content['picture'][1] }}" alt=""
                                             style="width: 810px;height: auto;display:none;">
                                    @endif
                                </div>
                            @elseif($template->type == 2)
                                <?php $content = $template->content ?>
                                <div id="template_{{ $template->order }}" class="main_view_content"
                                     data-img="{{ $content['popup'] }}">
                                    <!--banner-->
...
@endif
                                            ...
                                            <ul class="nav" style="margin-bottom: 40px;" role="tablist">
                                                @foreach($content['tab_title'] as $index => $value)
                                                    @if($value)
                                                        <li>
                                                            <a class="{{ ($index == 0)? 'active' : '' }} exam_tab"
                                                               data-toggle="tab"
                                                               href="#topic_{{$template->id}}_{{$index}}" role="tab"
                                                               aria-controls="topic_1"
                                            ...
                                            <div class="tab-content">
                                                @foreach($content['tab_content'] as $index => $value)
                                                    <div class="tab-pane fade {{ ($index == 0)? 'show active' : '' }}"
                                                         id="topic_{{$template->id}}_{{$index}}" role="tabpanel"
                                        ...
                                        <!--right-ad-->
                                            <div class="ad_img_right">
                                                <img src="{{ $content['ads_pic'] }}" alt=""
                                                     onclick="show_ads(this,{{$template->id}})">
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <div class="exam_finish">
                                ...
                                <div class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <button type="button" class="btn btn_cancel" onclick="change_template(1)">修改答案
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="question">
                        <div class="question_header">
                            <div style="position:relative;">
                                ...
                                <input type="button" value="返回" onclick="location.href='{{ url('groups') }}'"
                                       class="btn_function"
                            ...
                            @foreach($questions as $index => $question)
                                <div id="q_{{$question->id}}" data-index="{{$question->id}}" class="question_each">
                                    <div class="question_each_title">
                                        {{array_key_exists($question->id,$new_question_no)? $new_question_no[$question->id] : ''}}
                                        . {{$question->name}}
                                    </div>
                                            ...
                                            <div class="question_each_content_text_descrition">
                                                ...
                                                @elseif($question->type == 2)
                                                    ...
                                                    @foreach($items as $index=>$item)
                                                        <div class="answer_choose"><input type="radio"
                                                                                          name="answer[{{$question->id}}]"
                                                                                          value="{{chr(65+$index)}}">{{ $item }}
                                                        </div>
                                                    @endforeach
                                                    @if(count($items) >= 4 && 0)
                                                        <div class="answer_choose"><input type="radio"
                                                ...
                                                @elseif($question->type == 3)
                                                    <?php $items = json_decode($question->item, true) ?>
                                                    @foreach($items as $index=>$item)
                                                        <div class="answer_choose"><input type="checkbox"
                                                                                          name="answer[{{$question->id}}][]"
                                                                                          value="{{chr(65+$index)}}">{{ $item }}
                                                        </div>
                                                    @endforeach
                                                ...
                                                @if(array_key_exists($question->id,$question_step))
                                                    <div id="change_step">
                                                        <div class="save_btn"
                                                             onclick="change_template({{$question_step[$question->id]+1}})">
                                                            下一步
                                                        </div>
                                                        @if($question_step[$question->id]-1 != 0)
                                                            <div class="save_btn" style="margin-right:5px;"
                                                                 onclick="change_template({{$question_step[$question->id]-1}})">
                                                                上一步
                                                            </div>
                                                        @endif
